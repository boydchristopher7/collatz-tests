#!/usr/bin/env python3

# -------------------------------
# projects/collatz/TestCollatz.py
# Copyright (C) 2016
# Glenn P. Downing
# -------------------------------

# https://docs.python.org/3.4/reference/simple_stmts.html#grammar-token-assert_stmt

# -------
# imports
# -------

from io import StringIO
from unittest import main, TestCase

from Collatz import collatz_read, cycle_length, collatz_eval, collatz_print, collatz_solve

# -----------
# TestCollatz
# -----------


class TestCollatz (TestCase):
    # ----
    # read
    # ----
    
    # reading 2 integer values
    def test_read_1(self):
        s = "1 10\n"
        i, j = collatz_read(s)
        self.assertEqual(i,  1)
        self.assertEqual(j, 10)

    # reading more than 2 values
    def test_read_2(self):
        s="20 10 10\n"
        self.assertFalse(collatz_read(s))

    # reading non-integer values
    def test_read_3(self):
        s = "a b\n"
        self.assertFalse(collatz_read(s))

    # reading negative values    
    def test_read_4(self):
        s="-10 -20\n"
        self.assertFalse(collatz_read(s))


    # ------------------------------
    # cycle_length (Helper function)
    # ------------------------------
    
    def test_len_1(self):
        v = cycle_length(1)
        self.assertEqual(v,1)
    def test_len_2(self):
        v = cycle_length(1000)
        self.assertEqual(v,112)
    def test_len_3(self):
        v = cycle_length(50000)
        self.assertEqual(v,128)
 
    # ----
    # eval
    # ----

    def test_eval_1(self):
        v = collatz_eval(2, 10)
        self.assertEqual(v, 20)

    def test_eval_2(self):
        v = collatz_eval(1000, 1205)
        self.assertEqual(v, 182)
        
    def test_eval_3(self):
        v = collatz_eval(1001, 700)
        self.assertEqual(v, 179)

    def test_eval_4(self):
        v = collatz_eval(1000, 3005)
        self.assertEqual(v, 217)

    def test_eval_5(self):
        v = collatz_eval(1005, 8005)
        self.assertEqual(v, 262)

    def test_eval_6(self):
        v = collatz_eval(1, 1000000)
        self.assertEqual(v, 525)    



    # -----
    # print
    # -----

    def test_print_1(self):
        w = StringIO()
        collatz_print(w, 1, 10, 20)
        self.assertEqual(w.getvalue(), "1 10 20\n")
    
    def test_print_2(self):
        w = StringIO()
        collatz_print(w, 1000, 900, 174)
        self.assertEqual(w.getvalue(), "1000 900 174\n")
    # -----
    # solve
    # -----

    def test_solve_1(self):
        r = StringIO("1 10\n100 200\n\n201 210\n900 1000\n")
        w = StringIO()
        collatz_solve(r, w)
        self.assertEqual(
            w.getvalue(), "1 10 20\n100 200 125\n201 210 89\n900 1000 174\n")

# ----
# main
# ----

if __name__ == "__main__":
    main()

""" #pragma: no cover
% coverage-3.5 run --branch TestCollatz.py >  TestCollatz.out 2>&1


% cat TestCollatz.out
.......
----------------------------------------------------------------------
Ran 7 tests in 0.000s
OK


% coverage-3.5 report -m                   >> TestCollatz.out



% cat TestCollatz.out
.......
----------------------------------------------------------------------
Ran 7 tests in 0.000s
OK
Name             Stmts   Miss Branch BrPart  Cover   Missing
------------------------------------------------------------
Collatz.py          12      0      2      0   100%
TestCollatz.py      32      0      0      0   100%
------------------------------------------------------------
TOTAL               44      0      2      0   100%
"""
